...
def approve_question(qid):
    ...
    q = Question(question_text=uq.question_text, difficulty=uq.difficulty, category_id=uq.category_id, is_active=True)
    db.session.add(q)
    db.session.flush()
    ...
    if uq.user and uq.user.discord_account:
        try:
            requests.post(
                ...
                json={"discord_id": uq.user.discord_account.discord_id, "message": f"✅ Your question has been approved!\nQuestion: {uq.question_text[:50]}..."},
                timeout=5
            )
        except Exception as e:
...
def delete_shop_item(item_id):
    ...
    # Холбогдох UserInventory бичлэгүүдийг устгах
